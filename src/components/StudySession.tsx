    ...
    <div className="flex-1 flex flex-col h-full min-h-0 relative bg-editorial-bg">
      ...
      <header className="p-6 md:p-12 pb-0 flex flex-col sm:flex-row items-center gap-4 justify-between relative z-10 transition-colors">
        <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 md:gap-4 text-[8px] md:text-[10px] uppercase tracking-[0.25em] font-bold text-editorial-muted">
          ...
          <span className="hidden sm:inline w-8 h-[1px] bg-editorial-border"></span>
          {status === 'mastered' ? (
            <span className="text-emerald-500 dark:text-emerald-400 flex items-center gap-1 md:gap-2 font-black shrink-0"><CheckCircle2 size={10} /> Mastered</span>
          ) : status === 'review' ? (
            <span className="text-amber-500 dark:text-amber-400 flex items-center gap-1 md:gap-2 font-black shrink-0"><AlertCircle size={10} /> Review</span>
          ) : (
        ...
        <div className="text-[9px] md:text-[10px] font-mono text-editorial-meta bg-editorial-accent dark:bg-zinc-900 px-3 py-1.5 rounded-sm font-bold border border-editorial-border shadow-sm">
            ...
            >
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 md:gap-8 mb-6 md:mb-10">
                <div className="flex items-center gap-4 md:gap-6 overflow-visible">
                  <h2 
                    className="text-3xl sm:text-5xl md:text-7xl lg:text-[120px] font-serif leading-none tracking-tight text-editorial-text dark:text-editorial-accent select-all cursor-pointer hover:text-editorial-muted transition-colors break-words"
                    onClick={handleSpeak}
                  >
                    {word.word}
                  </h2>
                ...
                <div className="flex gap-2 md:gap-3 shrink-0">
                  <button 
                    ...
                    className={`flex-1 md:flex-none flex items-center justify-center p-3 md:p-4 rounded-sm border-2 transition-all duration-300 shadow-sm md:shadow-lg ${status === 'mastered' ? 'bg-editorial-text text-editorial-bg border-editorial-text' : 'bg-white dark:bg-zinc-900 text-editorial-meta border-editorial-border hover:border-editorial-text'}`}
                  ...
                  <button 
                    ...
                    className={`flex-1 md:flex-none flex items-center justify-center p-3 md:p-4 rounded-sm border-2 transition-all duration-300 shadow-sm md:shadow-lg ${status === 'review' ? 'bg-amber-600 dark:bg-amber-500 text-white border-amber-600 dark:border-amber-500' : 'bg-white dark:bg-zinc-900 text-editorial-meta border-editorial-border hover:border-editorial-text'}`}
                  ...
                  <div className="text-xl md:text-2xl leading-snug text-editorial-text font-medium">
                    ...
                    <div className="mt-8 space-y-8">
                      {word.derivatives && word.derivatives.length > 0 && (
                        <div className="bg-neutral-50 dark:bg-zinc-950 border border-editorial-border p-4 rounded-sm">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        {word.synonyms && word.synonyms.length > 0 && (
                          <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {word.synonyms.map(s => (
                                <span key={s} className="px-3 py-1 bg-editorial-accent dark:bg-zinc-800 text-[11px] font-bold tracking-tight border border-editorial-border rounded-sm hover:translate-y-[-1px] transition-transform cursor-default">
                                  {s}
                                </span>
                              ))}
                            </div>
                        ...
                        {word.antonyms && word.antonyms.length > 0 && (
                          <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {word.antonyms.map(a => (
                                <span key={a} className="px-3 py-1 bg-white dark:bg-zinc-900 text-[11px] font-bold tracking-tight border border-editorial-border rounded-sm italic text-editorial-muted hover:translate-y-[-1px] transition-transform cursor-default">
                                  {a}
                                </span>
                              ))}
                            </div>
      ...
      {/* Bottom Control Bar */}
      <section className="h-28 md:h-44 border-t-2 border-editorial-text bg-white dark:bg-zinc-950 relative z-10 shrink-0 transition-colors">
        <div className="flex h-full divide-x divide-editorial-border">
          <button 
            onClick={handlePrev}
            className="flex-1 p-3 md:p-8 flex flex-col justify-between hover:bg-editorial-accent dark:hover:bg-zinc-900 transition-all text-left group"
          >
          ...
          <div className="hidden xl:flex flex-[2] p-8 bg-editorial-accent dark:bg-zinc-900 overflow-hidden">
            <div className="w-full">
              ...
              <div className="grid grid-cols-3 gap-x-6 gap-y-2 overflow-hidden">
                {block.words.slice(Math.max(0, index - 4), index + 8).map((w, i) => (
                  <div 
                    key={i} 
                    className={`text-[11px] font-medium truncate flex items-center gap-2 ${w.word === word.word ? 'text-editorial-text dark:text-editorial-accent font-bold' : 'text-editorial-muted opacity-60'}`}
                  >
                    {w.word === word.word && <div className="w-1 h-1 bg-editorial-text dark:bg-editorial-accent rounded-full" />}
                    {w.word}
                  </div>
                ))}
              </div>
      ...
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-30 pointer-events-none w-full max-w-sm px-6">
        <motion.div 
          ...
          className="pointer-events-auto bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md border border-editorial-text/20 shadow-[0_20px_50px_rgba(0,0,0,0.15)] rounded-full px-6 py-3 flex items-center justify-between gap-4"
        >
          <button 
            onClick={handlePrev}
          ...
          <div className="flex items-center gap-2 border-x border-editorial-border px-4 mx-2">
            ...
            <button 
              onClick={() => onToggleBookmark(word.word)}
              className={`p-2 rounded-full transition-all ${isBookmarked ? 'bg-editorial-text text-white' : 'text-editorial-meta hover:bg-neutral-100 dark:hover:bg-zinc-800'}`}
              title="Bookmark (B)"
            >
        ...
        {bulkConfirm && (
          ...
          >
            <motion.div 
              ...
              className="bg-white dark:bg-zinc-900 p-8 md:p-12 max-w-sm w-full border border-editorial-border shadow-2xl rounded-sm text-center"
            >
              <div className="flex justify-center mb-6">
                {bulkConfirm === 'mastered' ? (
                  <div className="p-4 bg-emerald-50 dark:bg-emerald-500/10 rounded-full text-emerald-500 border border-emerald-100 dark:border-emerald-500/20">
              ...
              <h3 className="text-xl font-serif italic text-editorial-text mb-4 dark:text-white">Bulk Systematic Update</h3>
              ...
              <div className="flex flex-col gap-3">
                ...
                <button 
                  ...
                  className="w-full py-4 rounded-sm text-[10px] uppercase font-bold tracking-[0.2em] text-editorial-meta hover:text-editorial-text transition-colors bg-neutral-50 dark:bg-zinc-800"
                >
                  Cancel Operation
                </button>
              </div>
