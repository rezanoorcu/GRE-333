...
export const IdiomsSection: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-editorial-bg overflow-hidden">
      {/* Header */}
      <header className="px-8 py-10 border-b border-editorial-border bg-white shrink-0">
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center">
            <div className="relative w-full md:w-64">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 bg-neutral-50 border border-editorial-border text-xs focus:outline-none focus:border-editorial-text transition-all rounded-sm italic"
              />
            </div>
            <button 
              onClick={() => setShowRecallKeys(!showRecallKeys)}
              className="px-4 py-2 bg-editorial-text text-white text-[9px] uppercase font-bold tracking-widest rounded-sm flex items-center gap-2 hover:bg-neutral-800 transition-colors shrink-0"
            >
      ...
      {/* Categories Bar */}
      <div className="px-8 py-3 bg-white border-b border-editorial-border flex gap-4 overflow-x-auto no-scrollbar shrink-0">
        {categories.map(cat => (
          <button
            key={cat}
        ...
        <AnimatePresence mode="popLayout">
          {filteredIdioms.map((idiom, idx) => (
            <motion.div
              ...
              className="bg-white border border-editorial-border p-6 rounded-sm shadow-[0_4px_0_0_#f4f4f4] hover:shadow-[0_8px_0_0_#f0f0f0] transition-all flex flex-col group"
            >
      ...
      {/* Footer Info */}
      <footer className="px-8 py-4 bg-white border-t border-editorial-border text-[9px] uppercase font-bold tracking-widest text-editorial-meta flex justify-between shrink-0">
